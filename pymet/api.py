...
class Met:
    _path_to_csv = constants.CsvPaths.COLLECTION
...
class MetPaintings(Met):
    _path_to_csv = constants.CsvPaths.PAINTINGS

    def __init__(self):
        super().__init__()
        self.data = self._process_collection_data()
        self.lookup = self._create_artist_lookup()

    def _process_collection_data(self):
        data = self.data.copy()
    ...
    def _create_artist_lookup(self):
    ...
    def works_by_artist(self, artist):
